...
window.addEventListener('onorientationchange' in window ? 'orientationchange' : 'resize', function () {
    ...
    setTimeout(function () {
        ...
        if (window.isPortrait) {
            var _h2 = ($('body').height() / 2)
            $("#topHalf").removeAttr('style').css({"z-index": "1", "height": _h2})

            $("#bottomHalf").find(".slider-bd").removeAttr('style').css({
                "transition-duration": "0ms",
                "height": _h2 - 40
            })
            $("#chat_container").removeAttr('style').css("overflow","hidden")
        } else {
            var _h1 = ($('body').height() / 2)
            _h1 = _h1 - 40
            $("#bottomHalf").find(".slider-bd").removeAttr('style').css({"transition-duration": "0ms", "height": _h1})
        }
    }, 300);
